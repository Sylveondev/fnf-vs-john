...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'johnroll':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/johnroll/Dialogue'));
			case 'busta':
				...
				case 'dave':
					{
						defaultCamZoom = 0.9;
						curStage = 'dave';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave'));
						if(FlxG.save.data.antialiasing)
							{
								bg.antialiasing = true;
							}
		...
		switch (SONG.player2)
		{
			...
			case 'johninsane':
				if (FlxG.save.data.distractions)
				{
					...
					if (!PlayStateChangeables.Optimize)
					{
						var johntrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
						// johntrail.changeValuesEnabled(false, false, false, false);
						// johntrail.changeGraphic()
						add(johntrail);
					}
					// evilTrail.scrollFactor.set(1.1, 1.1);
				}
		...
		kadeEngineWatermark = new FlxText(4, healthBarBG.y
			...
			+ (Main.watermarks ? " | Kade Engine Suppremacy, version " + MainMenuState.kadeEngineVer : ""), 16);
